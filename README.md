...
# ShogiHome Recitation Fork（背譜モード追加版）

> ⚠️ これは [ShogiHome](https://github.com/sunfish-shogi/shogihome) に**「背譜モード（棋譜暗記・再現）」**を追加したフォークです。
> This is a fork of [ShogiHome](https://github.com/sunfish-shogi/shogihome) with an added **"Recitation Mode"** (memorizing and reproducing a game record).

## 背譜モードとは / About Recitation Mode

開いている棋譜を暗記対象とし、盤上で指した手を棋譜（メイン画面で選択中の分岐を含む活動経路）と照合します。

- 正解 → 緑色メッセージ（0.3 秒で自動クローズ）／不正解 → 赤色メッセージ（0.5 秒で自動クローズ）
- 手番モード：先手のみ / 後手のみ / 両方（片方のみの場合は相手の手が 0.4 秒の遅延で自動進行）
- ヒント：正解の手を自動で指す
- 「前の一手」で 1 手戻って復習できる
- 完成時に成績（総手数・誤り回数・経過時間）を表示
- 詳細は [specs/recitation-mode.md](./specs/recitation-mode.md)

## ダウンロード / Download

[Releases](https://github.com/Chtholly0721/shogihome-recitation/releases) からダウンロードできます。
