  ...
  <div>
    <dialog v-if="!isInitialPositionMenuVisible && !isGameMenuVisible" ref="dialog" class="menu">
      ...
      <div class="group">
        ...
        <button :disabled="!states.recitation" @click="onStartRecitation">
          <Icon :icon="IconType.MEMORY" />
          <div class="label">{{ t.recitationMode }}</div>
        </button>
...
const onStartRecitation = () => {
  store.startRecitation();
  emit("close");
};
const onExportImage = () => {
